...
// ✅ DEBUGGING VERSION - Check karo console mein kya aa raha hai
...
async function updateOrderDetails(orderId) {
    ...
    console.log("Full URL:", `${BASE_URL}/api/v1/user/updateOrder/${orderId}`);
    ...
    try {
        ...
        if (!response.ok) {
            ...
            try {
                ...
                alert(`Failed to update order: ${errorJson.message || errorJson.error || 'Unknown error'}`);
            } catch {
...
async function cancelOrder(orderId) {
    ...
    console.log("URL:", `${BASE_URL}/api/v1/user/cancelOrder/${orderId}`);
    ...
    try {
        const response = await fetch(`${BASE_URL}/api/v1/user/cancelOrder/${orderId}`, {
            ...
            body: JSON.stringify({ 
                ...
                message: "Seller canceled the order" 
            })
        });
