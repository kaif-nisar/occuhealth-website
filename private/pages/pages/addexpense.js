...
saveNewCategory.addEventListener('click', async () => {
    ...
    try {
        const res = await fetch(`${BASE_URL}/api/v1/user/addNewCategoryuser`, {
            method: 'POST',
            headers: {
...
async function populateCategories() {
    try {
        const res = await fetch(`${BASE_URL}/api/v1/user/getAllCategoriesuser`);
        const data = await res.json();
...
expenseForm.addEventListener('submit', async (e) => {
    ...
    try {
        const res = await fetch(`${BASE_URL}/api/v1/user/addNewExpenseuser`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
