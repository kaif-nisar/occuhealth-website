...
async function fetchExpensesByDateRange(startDate, endDate) {
    try {
        const res = await fetch(`${BASE_URL}/api/v1/user/fetchUserExpenses?startDate=${startDate}&endDate=${endDate}`);
...
editExpenseForm.addEventListener('submit', async (evt) => {
  ...
  try {
    // Send API request
    const response = await fetch(`${BASE_URL}/api/v1/user/editExpense/${editingId}`, {
      method: 'PUT',
      headers: {
